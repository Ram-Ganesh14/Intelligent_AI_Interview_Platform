...
import { FiZap, FiMail, FiLock, FiUser, FiArrowRight, FiEye, FiEyeOff } from "react-icons/fi";
...
export default function RegisterPage() {
    ...
    return (
        <div className="bg-grid-pattern" style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
            <div className="orb orb-purple" style={{ width: 400, height: 400, top: -80, right: -100 }} />
            <div className="orb orb-cyan" style={{ width: 300, height: 300, bottom: -60, left: -80 }} />
            ...
            <div className="glass-card animate-fade-in-up" style={{ width: "100%", maxWidth: 440, padding: "40px", position: "relative", zIndex: 1 }}>
                <div style={{ textAlign: "center", marginBottom: "32px" }}>
                    ...
                    <p style={{ marginTop: "12px", color: "var(--text-secondary)", fontSize: "0.92rem" }}>Create your account</p>
                </div>

                <div className="tab-nav" style={{ marginBottom: "24px" }}>
                    {["candidate", "recruiter", "admin"].map((r) => (
                        <button key={r} className={`tab-btn ${role === r ? "active" : ""}`} onClick={() => setRole(r)} style={{ flex: 1, textTransform: "capitalize" }}>{r}</button>
                    ))}
                </div>

                <form onSubmit={handleRegister}>
                    <div style={{ marginBottom: "16px" }}>
                        <label style={{ fontSize: "0.82rem", color: "var(--text-secondary)", fontWeight: 500, display: "block", marginBottom: "6px" }}>Full Name</label>
                        <div style={{ position: "relative" }}>
                            <FiUser size={16} style={{ position: "absolute", left: 14, top: 14, color: "var(--text-muted)" }} />
                            <input className="input-field" type="text" placeholder="Jane Doe" value={name} onChange={(e) => setName(e.target.value)} style={{ paddingLeft: 40 }} />
                        </div>
                    </div>
                    <div style={{ marginBottom: "16px" }}>
                    ...
                    <div style={{ marginBottom: "24px" }}>
                        ...
                        <div style={{ position: "relative" }}>
                            ...
                            <input className="input-field" type={showPw ? "text" : "password"} placeholder="Min 8 characters" value={password} onChange={(e) => setPassword(e.target.value)} style={{ paddingLeft: 40, paddingRight: 40 }} />
